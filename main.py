...
#     except Exception as e:
#         raise HTTPException(status_code=500, detail=f"Supabase insert/upsert failed: {e}")


# # ======================================================
# # ✅ Manual AI Query Endpoint
# # ======================================================
# @app.post("/agent-query")
# async def agent_query(req: QueryRequest, claims: dict = Depends(get_claims)):
#     org_id = claims.get("org_id")
#     try:
#         system_prompt = f"You are an AI assistant for org {org_id}. Provide concise, accurate insights."
...
#     try:
#         # -------------------------------
#         # 🔹 Try querying with RLS client (using anon key)
#         # -------------------------------
...
#                     return {"data": data}
#                 else:
#                     print(f"⚠️ [ROWS] No rows found via RLS for org_id={org_id}")
#             except Exception as e:
#                 print(f"⚠️ [ROWS] RLS query failed: {e}")
...
#         # -------------------------------
#         # 🔹 Final fallback: fetch all rows if org_id mismatch
#         # -------------------------------
#         print(f"⚠️ [ROWS] No rows found for org_id={org_id}. Fetching all rows instead...")
#         resp = service_role_client.table("sheets_rows").select("*").limit(limit).execute()
#         data = getattr(resp, "data", None) or []
#         print(f"✅ [ROWS] Fallback returned {len(data)} total rows.")
#         return {"data": data}

#     except Exception as e:
#         traceback.print_exc()
#         print(f"❌ [ROWS] Error fetching rows: {e}")
...
#                 )
#             except Exception as e:
#                 ai_result = f"(mocked fallback) {content} — error: {e}"
...
#         print(f"✅ Completed {len(processed)} rows for org_id={org_id}")
...
#     """Automatically runs the AI agent every 30 minutes."""
#     try:
#         org_id = "org_001"
#         print(f"🤖 [Scheduler] Running agent automation for {org_id}...")

#         rows = service_role_client.table("sheets_rows").select("*").eq("org_id", org_id).execute().data
#         if not rows:
#             print("📭 [Scheduler] No new rows to process.")
#             return

#         print(f"📦 [Scheduler] Found {len(rows)} rows. Running AI summarization...")
#         for row in rows:
#             row_id = row.get("sheet_row_id")
#             content = str(row.get("data"))
#             ai_result = f"(scheduled run) summary for {row_id}: {content[:50]}"
...
#     automated_agent_job()
#     return {"message": "Scheduler job executed manually."}
...
# def debug_supabase():
#     """Extra route to verify Supabase connectivity."""
#     try:
#         resp = service_role_client.table("sheets_rows").select("*").limit(1).execute()
#         data = getattr(resp, "data", [])
#         return {"ok": True, "sample_rows": data}
#     except Exception as e:
#         import traceback
#         traceback.print_exc()
...
# # ======================================================
# # ✅ Temporary Mock Data Seeder (for demo)
# # ======================================================
# import random
